...
import { getCommitSystem } from "../../utils/ai/generate.js";
...
export default eventHandler(async (event) => {
  ...
  const { changes } = await readBody(event);
  if (!changes) {
    throw createError({
      statusCode: 400,
  ...
  const messagePrefix = config.public.studio?.git?.commit?.messagePrefix || void 0;
